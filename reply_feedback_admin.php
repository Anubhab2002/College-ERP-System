...
<body>
    ...
    if (isset($_POST["submit"])) {
        ...
        $sql = "UPDATE Feedback SET feedback_reply='{$feedback_reply}' WHERE feedback_ID='{$feedback_ID}' AND feedback_to='{$_SESSION["admin_ID"]}'";
        if ($db->query($sql)) {
    ...
    ?>
    <?php
    $sql = "SELECT * FROM Feedback WHERE feedback_to='{$_SESSION["admin_ID"]}'";
    $result = $db->query($sql);
